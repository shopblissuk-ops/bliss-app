...
const TIPS = {
  ...
  all: [
    "biotin, zinc, and vitamins a, c, d and e all work synergistically.",
...
const s = {
  ...
  btn: { width: "100%", padding: "16px", background: PINK, color: "#fff", border: "none", borderRadius: 999, fontSize: 15, fontWeight: 500, fontFamily: "'DM Sans', sans-serif", cursor: "pointer" },
  ...
  label: { fontSize: 11, color: "#bbb", letterSpacing: "0.1em", textTransform: "uppercase", margin: "0 0 0.75rem" },
  ...
  tabBar: { position: "fixed", bottom: 0, left: "50%", transform: "translateX(-50%)", width: "100%", maxWidth: 480, background: "#fff", borderTop: "0.5px solid #f0e8e7", display: "flex", zIndex: 100 },
  tab: (a) => ({ flex: 1, padding: "10px 4px 14px", display: "flex", flexDirection: "column", alignItems: "center", gap: 3, cursor: "pointer", background: "none", border: "none", fontFamily: "'DM Sans', sans-serif", fontSize: 9, color: a ? PINK : "#bbb", letterSpacing: "0.04em", textTransform: "uppercase" }),
  goalCard: (sel) => ({ border: sel ? `1.5px solid ${PINK}` : "0.5px solid #f0e8e7", borderRadius: 14, padding: "1rem", cursor: "pointer", background: sel ? LIGHT : "#fff" }),
  waterBtn: (f) => ({ width: 36, height: 36, borderRadius: "50%", background: f ? PINK : "#f0e8e7", border: "none", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 16 }),
  ...
  leaderRow: (u) => ({ display: "flex", alignItems: "center", gap: "0.75rem", padding: "0.875rem 1rem", borderRadius: 12, background: u ? LIGHT : "transparent", border: u ? "0.5px solid #f0e8e7" : "none", marginBottom: 6 }),
};
...
function InstallPage({ onInstalled }) {
  ...
  const iosSteps = [
    { icon: "1", text: "tap the Share button at the bottom of your browser", img: "⬆️" },
    { icon: "2", text: "scroll down and tap \"Add to Home Screen\"", img: "➕" },
    { icon: "3", text: "tap \"Add\" in the top right corner", img: "✓" },
  ];

  const androidSteps = [
    { icon: "1", text: "tap the three dots menu in the top right of your browser", img: "⋮" },
    { icon: "2", text: "tap \"Add to Home screen\" or \"Install app\"", img: "➕" },
    { icon: "3", text: "tap \"Add\" to confirm", img: "✓" },
  ];

  const steps = isIOS ? iosSteps : androidSteps;
  const device = isIOS ? "iPhone" : isAndroid ? "Android" : "your phone";

  return (
    <div style={{ ...s.app, paddingBottom: "2rem" }}>
      <div style={{ background: `linear-gradient(135deg, ${PINK}, #e8c4c0)`, padding: "3rem 1.5rem 2.5rem", textAlign: "center" }}>
        <p style={{ fontFamily: "'Cormorant', Georgia, serif", fontSize: 32, color: "#fff", margin: "0 0 8px", fontWeight: 400 }}>bliss for you</p>
        <p style={{ fontSize: 13, color: "rgba(255,255,255,0.8)", margin: 0, letterSpacing: "0.05em", textTransform: "uppercase" }}>30 day glow tracker</p>
      </div>

      <div style={s.section}>
        <p style={{ ...s.label, marginBottom: "0.5rem" }}>free app</p>
        <h1 style={{ ...s.heading, fontSize: 30 }}>add to your home screen</h1>
        <p style={s.sub}>get the full tracker experience — daily reminders, streak tracking, and your rewards. takes 10 seconds on {device}.</p>

        <div style={{ display: "flex", flexDirection: "column", gap: 12, marginBottom: 28 }}>
          {steps.map((step, i) => (
            <div key={i} style={{ display: "flex", alignItems: "flex-start", gap: "1rem", padding: "1.1rem 1.25rem", border: "0.5px solid #f0e8e7", borderRadius: 14, background: "#fdfafa" }}>
              <div style={{ width: 32, height: 32, borderRadius: "50%", background: PINK, color: "#fff", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 14, fontWeight: 500, flexShrink: 0 }}>{step.icon}</div>
              <p style={{ fontSize: 14, color: DARK, margin: 0, lineHeight: 1.6 }}>{step.text}</p>
            </div>
          ))}
        </div>
        ...
        {isIOS && (
          <div style={{ ...s.card, textAlign: "center", marginBottom: 20 }}>
            <p style={{ fontSize: 13, color: "#999", margin: 0 }}>look for this icon at the bottom of Safari ⬆️</p>
          </div>
        )}

        <button style={s.btn} onClick={onInstalled}>
          I've added it to my home screen →
        </button>

        <p style={{ fontSize: 12, color: "#ccc", textAlign: "center", marginTop: 12 }}>no app store needed — completely free</p>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
          {[
            "30 day check-in tracker with streak counter",
            "personalised tips based on your goal",
            "water intake tracker",
            "before & after photo progress",
            "community leaderboard",
            "reorder reminders so you never run out",
          ].map((f, i) => (
...
export default function App() {
  ...
  function addWater() {
    const v = Math.min(WATER_GOAL, water + 1);
    setWater(v);
    localStorage.setItem("bliss_water_" + today, v.toString());
  }

  function removeWater() {
    const v = Math.max(0, water - 1);
    setWater(v);
    localStorage.setItem("bliss_water_" + today, v.toString());
  }
      ...
      <div style={s.section}>
        {step === 1 && (
          <>
            ...
            <h1 style={s.heading}>start your <span style={{ color: PINK }}>glow</span> journey</h1>
            <p style={s.sub}>track your daily gummies, build your streak, and watch your transformation over 30 days.</p>
        ...
        {step === 2 && (
          <>
            ...
            <p style={s.sub}>we'll personalise your tips based on what matters most to you.</p>
  ...
  return (
    <div style={s.app}>
      ...
      {tab === "tracker" && (
        <div style={s.section}>
          ...
          {alreadyCheckedIn || checkedIn ? (
            <div style={{ ...s.card, textAlign: "center", background: "#f5faf5", border: "0.5px solid #d4ecd4" }}>
              <p style={{ fontSize: 14, color: "#7db87d", margin: "0 0 4px" }}>checked in today</p>
            ...
            <>
              ...
              {msg && <p style={{ ...s.error, marginTop: 10 }}>{msg}</p>}
            </>
          )}
          <button onClick={handleShare} style={{ ...s.btnOutline, marginTop: 10 }}>{shareVisible ? "copied!" : "share my streak"}</button>
          <hr style={s.divider} />
          <p style={s.label}>your 30 days</p>
          <div style={s.grid}>
          ...
          <hr style={s.divider} />
          <p style={s.label}>water today</p>
          <p style={{ fontSize: 13, color: "#999", margin: "0 0 12px" }}>{water}/{WATER_GOAL} glasses</p>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
            {[{ label: "days done", value: streak }, { label: "days left", value: Math.max(0, 30 - streak) }, { label: "complete", value: Math.round((streak / 30) * 100) + "%" }, { label: "gummies taken", value: streak * 2 }].map((stat, i) => (
              <div key={i} style={{ ...s.card, textAlign: "center", marginBottom: 0 }}>
                <p style={{ fontSize: 24, fontFamily: "'Cormorant', Georgia, serif", color: PINK, margin: "0 0 4px", fontWeight: 400 }}>{stat.value}</p>
      ...
      {tab === "progress" && (
        <div style={s.section}>
          <h2 style={{ ...s.heading, fontSize: 28, marginBottom: "0.5rem" }}>your transformation</h2>
          <p style={s.sub}>document your before and after — stored only on your device.</p>
          <p style={s.label}>before & after</p>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginBottom: 20 }}>
            {["before", "after"].map(type => (
              <div key={type} style={{ borderRadius: 14, overflow: "hidden", border: "0.5px solid #f0e8e7", aspectRatio: "3/4", background: LIGHT, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", cursor: "pointer", position: "relative" }}
                ...
                {beforeAfter[type] ? <img src={beforeAfter[type]} style={{ width: "100%", height: "100%", objectFit: "cover" }} alt={type} /> : (<><p style={{ fontSize: 24, margin: "0 0 6px" }}>📷</p><p style={{ fontSize: 12, color: "#bbb", margin: 0 }}>tap to add {type}</p></>)}
          ...
          <hr style={s.divider} />
          <p style={s.label}>week by week</p>
          {[
            { week: "weeks 1-2", desc: "nutrients building up. working internally.", active: streak >= 1 },
            { week: "weeks 3-4", desc: "nails often show first. reduced shedding common.", active: streak >= 14 },
            { week: "weeks 5-6", desc: "skin clearer. hair feels thicker at roots.", active: streak >= 28 },
            { week: "week 8+", desc: "visible growth, stronger nails, consistent glow.", active: streak >= 30 },
          ].map((w, i) => (
            <div key={i} style={{ ...s.card, opacity: w.active ? 1 : 0.4, marginBottom: 10 }}>
      ...
      {tab === "leaderboard" && (
        <div style={s.section}>
          <h2 style={{ ...s.heading, fontSize: 28, marginBottom: "0.5rem" }}>leaderboard</h2>
          <p style={s.sub}>see how your streak compares to the bliss for you community.</p>
          <p style={s.label}>top streaks</p>
          ...
          {leaderboard.map((entry, i) => {
            ...
            return (
              <div key={i} style={s.leaderRow(isMe)}>
                <span style={{ fontSize: 16, width: 28, flexShrink: 0 }}>{medal}</span>
                <div style={{ flex: 1 }}>
                  <p style={{ fontSize: 14, fontWeight: isMe ? 500 : 400, color: DARK, margin: "0 0 2px" }}>{entry.name.split(" ")[0]}{isMe ? " (you)" : ""}</p>
          ...
          <hr style={s.divider} />
          <p style={s.label}>share your streak</p>
          <div style={{ border: "0.5px solid #f0e8e7", borderRadius: 20, overflow: "hidden" }}>
      ...
      {tab === "shop" && (
        <div style={s.section}>
          <h2 style={{ ...s.heading, fontSize: 28, marginBottom: "0.5rem" }}>get your next bottle</h2>
          <p style={s.sub}>never break your streak — order before you run out.</p>
          <div style={s.shopCard}>
            <div style={{ background: LIGHT, padding: "1rem 1.25rem", borderBottom: "0.5px solid #f0e8e7" }}>
            ...
            <div style={{ padding: "1.25rem" }}>
              ...
              <a href="https://blissforyou.uk" style={{ ...s.btn, display: "block", textAlign: "center", textDecoration: "none", marginTop: 16, lineHeight: "1.5", borderRadius: 999 }}>subscribe on blissforyou.uk</a>
            </div>
          </div>
          <div style={s.shopCard}>
            <div style={{ background: LIGHT, padding: "1rem 1.25rem", borderBottom: "0.5px solid #f0e8e7" }}>
            ...
            <div style={{ padding: "1.25rem" }}>
              ...
              <a href="https://blissforyou.uk" style={{ display: "block", textAlign: "center", textDecoration: "none", background: "transparent", color: PINK, border: `0.5px solid ${PINK}`, borderRadius: 999, padding: "16px", fontSize: 15, fontWeight: 500, fontFamily: "'DM Sans', sans-serif" }}>buy on blissforyou.uk</a>
            </div>
          </div>
          <div style={s.card}>
